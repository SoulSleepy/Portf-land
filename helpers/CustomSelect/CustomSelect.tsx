...
export const CustomSelect = ({
    ...
    const inputClasses =
        'outline-none bg-light rounded-md h-[30px] pl-1 w-full cursor-pointer outline-1 hover:outline-2 outline-text-light'
    const labelClasses =
        'bg-light text-sm absolute -top-[13px] left-1 leading-[17px]'
    const dropClasses =
        'absolute z-10 top-[35px] flex flex-col p-1 gap-1 min-h-min max-h-48 w-full outline outline-1 hover:outline-2 outline-text-light rounded-md bg-light overflow-auto '
                ...
                <div className={dropClasses}>
                    {arrOptions.map((item) => (
                        <div
                            key={item.name}
                            className={cn(
                                'flex flex-row gap-2 h-full w-full items-center cursor-pointer hover:bg-light-lighter',
                                {
                                    ['bg-light-lighter']:
