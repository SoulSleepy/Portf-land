...
export const Update = () => {
    ...
    const [getVersion, { data: dataVersion, isLoading: isLoadVersion }] = useLazyGetSystemVersionQuery()
    const [getChangeUpdate] = useLazyGetChangeUpdateQuery()

    useEffect(() => {
        if (dataInfo) {
            setUpadateValue(dataInfo['update-settings'].updating)
            setTimeValue(dataInfo['update-settings'].times?.join() as string)
        }
    }, [dataInfo])
    ...
    const saveSystem = () => {
        const paramsAuto = {times: timeValue.split(',').map(item => +item), updating: updateValue}
    ...
    const settingsBtnClasses =
        'flex bg-light-lighter hover:font-medium outline outline-0 hover:outline-1 h-10 w-[100px] items-center justify-center rounded-sm'
    const hrClasses = 'border-none bg-text-light h-[1.5px] w-full'

    return (
        <div className={blockClasses}>
            <p className={titleClasses}>
                Версия системы - {isLoadInfo ? 'loading' : dataInfo?.version}
            </p>
