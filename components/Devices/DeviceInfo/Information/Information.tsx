...
export const Information = ({ deviceId }: IProps) => {
    ...
    return (
        <div className='flex flex-col gap-3 overflow-auto h-[390px]'>
            <Resume resume={data?.resume ? data?.resume : null}/>
            {data?.agentInfo?.users.length && <UsersDevice users={data?.agentInfo?.users}/>}
            {data?.agentInfo?.main && <Basic main={data?.agentInfo?.main}/>}
            <Ports ports={data?.ports ? data?.ports : null} />
            {data?.agentInfo?.eq && <Equipment equipment={data?.agentInfo?.eq}/>}
            {data?.agentInfo?.usbDevices.length && <USBDevices usbDevices={data?.agentInfo?.usbDevices}/>}
        </div>
    )
}
